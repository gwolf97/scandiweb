...
const CartPageProduct = (props) => {
    ...
    let attributeSquares = props.selectedAttributes.map(obj => obj !== "" && <div  key={nanoid()} className={obj.attribute === "No" ? "hide" : "mini-size size product-att dark"}>{obj.attribute === "Yes" ? `${obj.id === 'Touch ID in keyboard' ? "Touch ID" : "USB x 3"}` : obj.attribute} </div>)

    return ( 
        <div key={nanoid()} className="mini-product product">
